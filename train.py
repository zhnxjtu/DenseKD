...
parser.add_argument('--kd-loss-weight', type=float, default=1.0,
                    help='dense patch kd loss weight')
...
parser.add_argument('--cuda-idx', type=str, default='0',
                    help='the index of cuda')
parser.add_argument('--instance-tmp', type=float, default=0.75,
                    help='instance temperature')
parser.add_argument('--patch-size', type=int, default=8,
                    help='patch size')

# TODO: add by zhanghn, 2024-5-4
parser.add_argument('--si-reverse', action='store_true', default=False, help='use sample importance reverse')
parser.add_argument('--reverse-type', type=str, default='dir', help='')
parser.add_argument('--prohi-att-map', action='store_true', default=False, help='use region importance')
parser.add_argument('--prohi-instance-map', action='store_true', default=False, help='use sample importance')
parser.add_argument('--shrink', type=int, default=None, help='use sample importance')
parser.add_argument('--att-overlap', action='store_true', default=False)
parser.add_argument('--stride', type=int, default=None, help='')
...
from model.patchkd import build_den_patch_kd, patch_at_loss, attention, attention_overlap, instance_imp, instance_imp_reverse, patch_at_loss_vanilla, patch_at_loss_instance
...
# model
if teacher is not None:
    cnn = build_den_patch_kd(args.model, num_classes=num_classes, teacher = args.teacher, shrink_input=args.shrink)
...
if 'shufflev1' in args.model or 'mobile' in args.model:
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if teacher is not None:
            ...
            # s_features, pred, norm_sum = cnn(images)
            ...
            """ attention map """
            if args.prohi_att_map:
                att_map = None
            else:
                if args.att_overlap:
                    att_map = attention_overlap(teacher, images, t_features[-1], patch_size=args.patch_size, stride=args.stride, mode='cosin')
                else:
                    att_map = attention(teacher, images, t_features[-1], patch_size=args.patch_size, mode='cosin')

            if args.prohi_instance_map:
                instance_weight = None
            else:
                if not args.si_reverse:
                    instance_weight = instance_imp(t_pred, labels, temp=args.instance_tmp)
                else:
                    instance_weight = instance_imp_reverse(t_pred, labels, temp=args.instance_tmp, mode=args.reverse_type)

            """ attention loss """
            if att_map is not None and instance_weight is not None:
                feature_kd_loss = patch_at_loss(s_features, t_features, att_map, instance_weight)
            elif att_map is None and instance_weight is not None:
                feature_kd_loss = patch_at_loss_instance(s_features, t_features, instance_weight)
                
            else:
                feature_kd_loss = patch_at_loss_vanilla(s_features, t_features)
            losses['DenCon_kd_loss'] = feature_kd_loss * min(1, epoch/args.kd_warm_up) * args.kd_loss_weight
    ...
    row = { 'epoch': str(epoch),
            'patch_size': '%d'%(args.patch_size),
            'instance_temp': '%.2f'%(args.instance_tmp),
