...
class ABF_wide(nn.Module):
    def __init__(self, in_channel, out_channel, in_channels, shrink):
        ...
        self.conv_refine0 = nn.Sequential(nn.Conv2d(in_channels[0], in_channels[0]//shrink[0], kernel_size=1, bias=False),
                                          nn.BatchNorm2d(in_channels[0]//shrink[0]),
        )
        nn.init.kaiming_uniform_(self.conv_refine0[0].weight, a = 1)

        self.conv_refine1 = nn.Sequential(nn.Conv2d(in_channels[1], in_channels[1]//shrink[1], kernel_size=1, bias=False),
                                          nn.BatchNorm2d(in_channels[1]//shrink[1]),
                                          )
        nn.init.kaiming_uniform_(self.conv_refine1[0].weight, a=1)

        self.conv_refine2 = nn.Sequential(nn.Conv2d(in_channels[2], in_channels[2]//shrink[2], kernel_size=1, bias=False),
                                          nn.BatchNorm2d(in_channels[2]//shrink[2]),
                                          )
        nn.init.kaiming_uniform_(self.conv_refine2[0].weight, a=1)

        self.conv_refine3 = nn.Sequential(nn.Conv2d(in_channels[3], in_channels[3]//shrink[3], kernel_size=1, bias=False),
                                          nn.BatchNorm2d(in_channels[3]//shrink[3]),
                                          )
    ...
    def __init__(
        ...
        if shrink is not None:
            ...
            for in_num in range(len(in_channels)):
                in_channel = in_channel + in_channels[in_num]//shrink[in_num]
        else:
            in_channel = sum(in_channels[:])
