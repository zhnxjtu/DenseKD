...
from model.patchkd import build_den_patch_kd, patch_at_loss, attention, instance_imp, instance_imp_reverse, \
    patch_at_loss_vanilla
...
if args.test:
    cnn.load_state_dict(torch.load(args.resume)['model'])
        ...
        if teacher is not None:
            ...
            if args.prohi_instance_map:
                ...
                if not args.si_reverse:
                    ...
                    instance_weight = instance_imp_reverse(t_pred, labels, temp=args.instance_tmp,
                                                           mode=args.reverse_type)

            """ attention loss """
            if att_map is not None and instance_weight is None:
